...
`sigma_max`, `sigma_mean`, `sigma_free_eV_per_A` and `n_free_atoms` are all
taken over **free force components only** — the same population ASE's
`fmax` uses for its convergence test, not every atom in the cell. A
constrained atom cannot move regardless of how much the members disagree
...
If every atom in the structure is fully constrained, `sigma_max` and
`sigma_mean` fall back to the unmasked values (matching `sigma_max_all` /
`sigma_mean_all`) since there is no free population left to reduce over.

The pre-masking maximum survives as `sigma_max_all_eV_per_A` (trace CSV) /
`sigma_max_all_final_eV_per_A` (run record) so runs from before 2026-09-08
stay comparable. The per-atom CSV's unmasked value is `sigma_eV_per_A`,
alongside the new masked `sigma_free_eV_per_A`.
...
| `sigma_mean_eV_per_A` | Mean per-atom force disagreement at this step, over the atoms with at least one free component |
| `worst_atom` | Index of the atom with the largest (free-component) disagreement at this step |
...
`worst_atom` above already traces where the disagreement lived during the
run). This is usually the most diagnostically useful committee output. It
...
| `sigma_eV_per_A` | Per-atom force disagreement, unmasked (all three components) |
...
atoms alongside `sigma_eV_per_A` and `sigma_free_eV_per_A` side by side is
how a reader checks, on their own run, how much of the disagreement sits in
a region that cannot move.
...
**There is no default threshold.** The run always reports `sigma_max`,
`sigma_mean`, the worst atom, and `sigma_max_over_fmax_final` (the ratio of
`sigma_max` to the final `fmax`) — those numbers are the deliverable whether
or not anyone sets a threshold. `--uncertainty-threshold` (eV/Å) is
**opt-in**: pass it and a configuration is **flagged** when `sigma_max` at
the *final* geometry exceeds it. The reasoning is that if the members
disagree about the forces by more than the convergence tolerance, the
located minimum sits inside the committee's own noise. When flagged, the
CLI prints one warning naming the worst atom.
...
from `sigma_max`, so they are not a calibration for a new default either.
...
2026-09-08 sigma-masking change — a schema-4 record has the same key names
but the pre-masking `sigma_max` and a `threshold_source` of `"fmax"` or
`"explicit"`.
...
- `results.committee_uncertainty`: `n_steps`, `threshold_eV_per_A`,
  ...
  `sigma_max_final_eV_per_A`, `sigma_mean_final_eV_per_A`,
  `sigma_max_all_final_eV_per_A` (the pre-masking maximum, for comparison
  against runs from before 2026-09-08), `sigma_max_peak_eV_per_A`,
  `sigma_max_over_fmax_final` (`sigma_max_final_eV_per_A` divided by the
  final `fmax`, or `null` when there are no trace rows or the final `fmax`
  is exactly zero), `peak_step`,
  `worst_atom`, `worst_atom_symbol`, `n_free_atoms`, `unhandled_constraints`
